...
import { getContent, type Locale } from '../data';
import { dotFieldSvg, loadFont, readPalette } from './brand';
 ...
 * copy as the page itself, once per language. Satori only reads woff, so the
 * display face comes from the static @fontsource package rather than the
 * variable one the site loads; both are Bricolage Grotesque.
 */
export async function renderShareImage(locale: Locale): Promise<Response> {
